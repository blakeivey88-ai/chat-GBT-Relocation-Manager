...
let loadCatalog = [];
let loadCatalogError = "";
let loadCatalogLoaded = false;
let loadCatalogPromise = null;
let postedLoadCatalog = [];
let postedLoadCatalogLoaded = false;
let postedLoadCatalogPromise = null;
let latestDispatchPlan = null;
let postPhotoPayload = [];
...
async function resizeFreightPhoto(file) {
  ...
  while (dataUrl.length > 330_000 && quality > 0.48) {
    quality -= 0.08;
    dataUrl = canvas.toDataURL("image/jpeg", quality);
  }
  if (dataUrl.length > 350_000) {
    throw new Error("This photo is still too detailed after resizing. Choose a simpler or smaller image.");
  }
  return {
    name: String(file.name || "Freight photo").slice(0, 100),
    type: "image/jpeg",
    alt: "Freight condition photo",
    dataUrl,
  };
}
...
function renderPostPhotoPreview() {
  ...
  if (!preview) return;
  preview.innerHTML = postPhotoPayload.length
    ? postPhotoPayload
        .map(
          (photo, index) =>
            `<figure><img src="${escapeHtml(photo.dataUrl)}" alt="Freight photo ${index + 1}" /><figcaption>${escapeHtml(photo.name)}</figcaption></figure>`,
        )
        .join("")
    : '<span class="muted">No freight photos selected.</span>';
}
function normalizeLoadRecord(load = {}, index = 0) {
  ...
  return {
    ...
    postedByLogoUrl: String(load.postedByLogoUrl || "").trim(),
    tags: Array.isArray(load.tags) ? load.tags.slice(0, 12) : [],
    photos: Array.isArray(load.photos)
      ? load.photos.slice(0, 3).map((photo) => ({
          id: String(photo?.id || "").trim(),
          name: String(photo?.name || "Freight photo").trim(),
          type: String(photo?.type || "image/jpeg").trim(),
          alt: String(photo?.alt || "Freight condition photo").trim(),
        })).filter((photo) => photo.id)
      : [],
    myBid: load.myBid && typeof load.myBid === "object"
      ? {
          id: String(load.myBid.id || "").trim(),
          amount: Number(load.myBid.amount || 0) || 0,
          note: String(load.myBid.note || "").trim(),
          status: String(load.myBid.status || "pending").trim().toLowerCase(),
          updatedAt: String(load.myBid.updatedAt || load.myBid.requestedAt || "").trim(),
        }
      : null,
    agreedRate: Number(load.agreedRate || 0) || 0,
    claimRequests: Array.isArray(load.claimRequests)
      ? load.claimRequests.slice(0, 50).map((request) => ({
          ...
          updatedAt: String(request?.updatedAt || request?.requestedAt || "").trim(),
        }))
      : [],
...
async function loadPostedLoadCatalog(force = false) {
  if (adminPreviewMode) return [];
  if (postedLoadCatalogLoaded && !force) return postedLoadCatalog;
  if (postedLoadCatalogPromise && !force) return postedLoadCatalogPromise;
  postedLoadCatalogPromise = (async () => {
    try {
      const data = await apiRequest("/api/loads?scope=posted", {
        method: "GET",
      });
      postedLoadCatalog =
        data?.ok && Array.isArray(data.loads) ? data.loads : [];
    } catch {
      postedLoadCatalog = [];
    }
    postedLoadCatalogLoaded = true;
    renderPostedLoadResponses();
    return postedLoadCatalog;
  })().finally(() => {
    postedLoadCatalogPromise = null;
  });
  return postedLoadCatalogPromise;
}
let trustedPartners = [
  {
...
function regionFromText(value = "") {
  ...
  const stateMatch = parseRankState(text);
  if (stateMatch) return stateRegion(stateMatch);
  if (
    /\b(new england|northeast|mid-atlantic|tri-state)\b/.test(text)
  )
    return "Northeast";
  if (/\b(midwest|great lakes|heartland)\b/.test(text)) return "Midwest";
  if (/\b(south|southeast|gulf coast)\b/.test(text)) return "South";
  if (/\b(west|west coast|pacific|mountain)\b/.test(text)) return "West";
  return "";
}
...
const storageKeys = {
  ...
  directRequests: "rm_direct_requests",
...
function requestedAdminPreviewRole() {
  const requested = String(
    new URLSearchParams(location.search).get("preview") || "",
  )
    .trim()
    .toLowerCase();
  if (requested === "trucker") return "driver";
  return ["driver", "broker", "customer"].includes(requested) ? requested : "";
}
function adminPreviewProfile(role = "driver") {
  const common = {
    userId: `preview_${role}`,
    email: `${role}@preview.relocationmanagerusa.com`,
    emailVerifiedAt: new Date().toISOString(),
    subscriptionStatus: "active",
    subscriptionCurrentPeriodEnd: "Preview mode",
    profileComplete: true,
    verification: "Approved preview",
    dotMcStatus: "Verified",
    idCheckStatus: "Verified",
    insuranceStatus: "Verified",
    customerBrokerStatus: "Verified",
    ...
    memberAccess: {
      ...
      carrierLoadBookingAccess: {
        allowed: true,
        verified: true,
        message: "Approved for preview booking.",
      },
    },
    tags: ["Owner preview", "Demo data only"],
  };
  if (role === "broker") {
    return {
      ...common,
      name: "Blake — Broker Preview",
      company: "Preview Brokerage LLC",
      type: "Broker 4–7 trucks - $79.99/mo",
      role: "Broker",
      truckCount: 5,
      mc_dot: "MC-PREVIEW-5678",
      paymentStatus: "paid_fleet_growth",
      planLabel: "Fleet Growth",
      loadAccess: "claim_post",
      subscriptionAccess: "claim_post",
      note: "Safe owner preview of the broker and fleet workflow.",
    };
  }
  if (role === "customer") {
    return {
      ...common,
      name: "Blake — Shipper Preview",
      company: "Preview Shipper Co.",
      type: "Customer needing pickup (request-only) - $9.99/mo",
      role: "Customer / Shipper",
      truckCount: 0,
      mc_dot: "",
      paymentStatus: "paid_shipper",
      planLabel: "Shippers Plan",
      ...
      memberAccess: {
        ...
        carrierLoadBookingAccess: {
          allowed: false,
          verified: false,
          message: "Shippers can post loads but cannot claim them.",
        },
      },
      note: "Safe owner preview of the shipper posting workflow.",
    };
  }
  return {
    ...common,
    name: "Blake — Trucker Preview",
    company: "Preview Transport LLC",
    type: "Independent driver / self-insured - $29.99/mo",
    role: "Truck Driver",
    truckCount: 1,
    mc_dot: "DOT-PREVIEW-1234",
    paymentStatus: "paid_driver",
    planLabel: "Independent Driver",
    loadAccess: "claim_post",
    subscriptionAccess: "claim_post",
    note: "Safe owner preview of the trucker and owner-operator workflow.",
  };
}
function activateAdminPreview(role) {
  adminPreviewMode = true;
  adminPreviewRole = role;
  document.documentElement.classList.add("admin-preview");
  const profile = adminPreviewProfile(role);
  writeJSON(storageKeys.profile, profile);
  writeJSON(storageKeys.profileView, role);
  writeJSON(storageKeys.recentLoads, [
    {
      title: "Atlanta, GA → Nashville, TN",
      detail: "26 ft box truck · $685 · pickup tomorrow at 8:00 AM",
    },
  ]);
  writeJSON(
    storageKeys.recentRequests,
    role === "customer"
      ? [
          {
            title: "Atlanta, GA → Charlotte, NC",
            detail: "8 pallets · liftgate required · ready tomorrow",
          },
        ]
      : [],
  );
  writeJSON(
    storageKeys.activePickups,
    role === "customer"
      ? []
      : [
          {
            id: "preview_pickup_1",
            loadId: "preview_load_atl_nsh",
            title: "Atlanta, GA → Nashville, TN",
            detail: "26 ft box truck · pickup confirmed for tomorrow",
            status: "Confirmed",
            serverAuthorized: true,
            savedAt: new Date(Date.now() - 35 * 60 * 1000).toISOString(),
            statusHistory: [
              {
                status: "Confirmed",
  ...
  writeJSON(storageKeys.loadHistory, [
    {
      id: "preview_history_posted",
      loadId: "preview_load_atl_nsh",
      eventType: "posted",
      title: "Atlanta, GA → Nashville, TN",
      detail: "Pickup details posted with equipment and handling notes.",
      occurredAt: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000).toISOString(),
    },
    {
      id: "preview_history_viewed",
      loadId: "preview_load_atl_nsh",
      eventType: "viewed",
      title: "Atlanta, GA → Nashville, TN",
      detail: "Load details reviewed.",
      occurredAt: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString(),
    },
    {
      id: "preview_history_requested",
      loadId: "preview_load_atl_nsh",
      eventType: "pickup_requested",
      title: "Atlanta, GA → Nashville, TN",
      detail: "Pickup requested at the carrier's proposed rate.",
      occurredAt: new Date(Date.now() - 70 * 60 * 1000).toISOString(),
    },
    {
      id: "preview_history_accepted",
      loadId: "preview_load_atl_nsh",
      eventType: "accepted",
      title: "Atlanta, GA → Nashville, TN",
      detail: "Pickup request accepted by the posting shipper.",
      occurredAt: new Date(Date.now() - 35 * 60 * 1000).toISOString(),
    },
  ]);
  writeJSON(storageKeys.bulletinBoard, [
    {
      id: "preview_road_talk_1",
      previewOnly: true,
      authorUserId: "preview_blue_ridge",
      authorName: "Blue Ridge Logistics",
      authorRole: "Verified broker",
      accentColor: "#1d4ed8",
      language: "en",
      subject: "Lane update",
      body: "Atlanta to Nashville box-truck load opens tomorrow at 8 AM. @PreviewTransport reply if you have capacity.",
      likedBy: ["preview_member_1", "preview_member_2"],
      replies: [
        {
          id: "preview_reply_1",
          authorUserId: "preview_driver_reply",
          authorName: "Peachtree Freight",
          authorRole: "Carrier",
          accentColor: "#0f766e",
          body: "We may have a 26 ft truck empty in Atlanta. Sending the details now.",
          language: "en",
    ...
    {
      id: "preview_road_talk_2",
      previewOnly: true,
      authorUserId: "preview_verified_moves",
      authorName: "Verified Moves Co.",
      authorRole: "Moving company",
      accentColor: "#7c3aed",
      language: "en",
      subject: "Equipment needed",
      body: "Need liftgate help for eight pallets near Charlotte Friday morning. Tag a trusted carrier who covers this lane.",
      likedBy: ["preview_member_3"],
      replies: [],
      translations: {},
  ...
  writeJSON(storageKeys.plannedTrips, [
    {
      title: role === "broker" ? "Fleet capacity check" : "Return lane",
      origin: "Nashville, TN",
      destination: "Atlanta, GA",
      date: "Friday",
      equipment: "26 ft box truck",
      notes: "Owner preview data",
    },
  ]);
  return profile;
}
function renderAdminPreviewBanner(role) {
  if (document.getElementById("adminPreviewBanner")) return;
  const labels = {
    driver: "Trucker / Owner-Operator",
    broker: "Broker / Fleet",
  ...
  banner.setAttribute("aria-label", "Owner preview controls");
  banner.innerHTML = `<div><strong>Owner Preview: ${labels[role]}</strong><span>You can explore the full workflow. Preview actions do not change live accounts, loads, billing, or customer data.</span></div><div class="admin-preview-actions"><a href="/member.html?preview=driver#workbench">Trucker view</a><a href="/member.html?preview=broker#workbench">Broker view</a><a href="/member.html?preview=customer#workbench">Shipper view</a><a class="admin-preview-exit" href="/admin">Return to Admin</a></div>`;
  document.body.prepend(banner);
}
function adminPreviewMutationResponse(path, body = {}) {
  if (path === "/api/loads") {
    return {
      ok: true,
      preview: true,
      load: {
        id: `preview_load_${Date.now()}`,
        status: body.action === "claim" ? "Pickup requested" : "Open",
      },
    };
  }
  if (path === "/api/bulletin") {
    ...
    if (action === "reply") {
      ...
      const reply = {
        id: `preview_reply_${Date.now()}`,
        authorUserId: getProfile().userId,
        authorName: getProfile().company || getProfile().name,
        authorRole: profileRoleLabel(getProfile().type),
        accentColor: bulletinAccent(body.accentColor),
        body: String(body.body || ""),
        language: body.language || "en",
        createdAt: new Date().toISOString(),
      };
      if (post) post.replies = [...(post.replies || []), reply];
      saveBulletinPosts(posts);
      return { ok: true, preview: true, post, reply, posts };
    }
    if (action === "react") {
      const post = posts.find((item) => item.id === body.id);
      const userId = getProfile().userId;
      const likedBy = new Set(post?.likedBy || []);
      likedBy.has(userId) ? likedBy.delete(userId) : likedBy.add(userId);
      if (post) post.likedBy = [...likedBy];
      saveBulletinPosts(posts);
      return { ok: true, preview: true, post, posts };
    }
    const post = {
      ...(body.post || {}),
      id: `preview_post_${Date.now()}`,
      authorUserId: getProfile().userId,
      authorLogoUrl: getProfile().logoUrl || "",
      likedBy: [],
      replies: [],
      createdAt: new Date().toISOString(),
      preview: true,
  ...
  if (path === "/api/account" && body.action === "save") {
    return {
      ok: true,
      preview: true,
      profile: { ...getProfile(), ...(body.profile || {}) },
      profileView: adminPreviewRole,
    };
  }
  const error = new Error(
    "Preview mode does not change live customer, account, payment, or communication data.",
  );
  error.data = { error: error.message, preview: true };
  throw error;
}
function getProfile() {
...
function accountSnapshot() {
  ...
  const editableProfile = {
    ...
    insurancePolicyLast4: profile.insurancePolicyLast4,
    ...
    preferredTranslationLanguage: profile.preferredTranslationLanguage,
    autoTranslateMessages: profile.autoTranslateMessages,
    alwaysShowOriginalMessages: profile.alwaysShowOriginalMessages,
    transcribeAndTranslateVoiceNotes: profile.transcribeAndTranslateVoiceNotes,
    showLanguagesSpoken: profile.showLanguagesSpoken,
    ...
    bulletinColor: profile.bulletinColor,
  };
  return {
...
function mergeAccountState(data) {
  ...
  const carrierLoadBookingAccess =
    ...
    data.profile?.memberAccess?.carrierLoadBookingAccess ||
    null;
  if (data.profile)
    writeJSON(
      storageKeys.profile,
      Object.assign(profile, data.profile, {
        ...
        checkoutPlan:
          data.checkoutPlan ||
          data.profile.checkoutPlan ||
          profile.checkoutPlan ||
          "",
        subscriptionPriceCents:
          carrierLoadBookingAccess?.priceCents ||
          data.profile.subscriptionPriceCents ||
          profile.subscriptionPriceCents ||
          null,
        subscriptionStatus:
          ...
          "unpaid",
        subscriptionAccess:
          data.memberAccess?.subscriptionAccess ||
...
async function apiRequest(
  ...
  if (
    ...
    !(
      path === "/api/account" &&
      ["login", "register", "request-reset", "reset-password"].includes(
        String(body?.action || "").toLowerCase(),
      )
    )
  ) {
    return adminPreviewMutationResponse(path, body);
  }
  ...
  const publicAuthAction = String(body?.action || "").toLowerCase();
  if (
    res.status === 403 &&
    data?.csrfToken &&
    ["register", "login", "request-reset", "reset-password"].includes(
      publicAuthAction,
    )
  ) {
...
async function completeProfileAccount(profile) {
  const data = await apiRequest("/api/account", {
    method: "POST",
    body: {
...
function continueAfterAccountSetup(
  ...
  if (dashboardRoute === "verify" || accessRoute === "verify") {
    route("verify");
    return;
  }
  if (
    dashboardRoute === "plan-selection" ||
    accessRoute === "pricing"
  ) {
    location.href = planHref;
    return;
  }
  if (
    dashboardRoute === "renewal" ||
...
function profileTypeLabel(type = "") {
  ...
  if (/admin/.test(text)) return "Administrator";
  if (/customer|shipper|pickup/.test(text)) return "Customer / Shipper";
  if (/broker 1–3/.test(text)) return "Carrier & Broker 1–3 trucks";
  if (/broker 4–7/.test(text)) return "Carrier & Broker 4–7 trucks";
  if (/broker [78]–12/.test(text)) return "Carrier & Broker 8–12 trucks";
  if (/owner[- ]?operator|self[- ]?insured|independent driver/.test(text))
    return "Owner-Operator";
...
function isRequestOnlyAccount(profile = getProfile()) {
  ...
  const checkoutPlan = String(profile.checkoutPlan || "").toLowerCase();
  const accesses = [profile.loadAccess, profile.subscriptionAccess]
    .map((value) => String(value || "").toLowerCase())
    .filter(Boolean);
  const directPriceValues = [
    profile.subscriptionPriceCents,
    profile.billingAmountCents,
    profile.planAmountCents,
    profile.priceCents,
    profile.stripePriceAmountCents,
    profile.stripePriceUnitAmount,
  ];
  const hasShipperPrice = directPriceValues.some((value) => {
    const text = String(value || "").trim();
    if (!text) return false;
    const numeric = Number(text);
    if (!Number.isFinite(numeric) || numeric <= 0) return false;
    const cents =
      /^\d+\.\d{1,2}$/.test(text) || (Number.isInteger(numeric) && numeric < 100)
  ...
  return (
    ...
    accesses.some((access) =>
      ["request", "request_post", "post_only"].includes(access),
    )
  );
}
...
function pickupLimit(profile = getProfile()) {
  if (isRequestOnlyAccount(profile)) return 0;
  ...
  if (
    ...
    return 0;
  // Mirror of the server rule: plan truck count = concurrent pickup slots.
  if (
  ...
  if (payment === "paid_dispatcher_broker" || /dispatch/.test(type)) return 12;
  const truckCount = truckCountFromProfile(profile);
  return truckCount > 0 ? truckCount : 1;
}
function activePickupCount(profile = getProfile()) {
  if (isRequestOnlyAccount(profile)) return 0;
  return readJSON(storageKeys.activePickups, []).filter(
    (item) => item && item.serverAuthorized === true,
  ).length;
}
function pickupStatus(profile = getProfile()) {
  const limit = pickupLimit(profile);
  const active = activePickupCount(profile);
...
function bookingVerificationReason(profile = getProfile()) {
  ...
  return bookingVerificationReady(profile)
    ...
    : "Complete the checks that apply to your role: identity and insurance for most users; DOT/MC authority only when you operate under commercial authority, plus customer/broker verification and approval before booking.";
}
...
function isAdminPreviewBulletin(post) {
  return (
    Boolean(post?.previewOnly) ||
    String(post?.id || "").startsWith("preview_") ||
    String(post?.authorUserId || "").startsWith("preview_")
  );
}
function bulletinPosts() {
  const posts = readJSON(storageKeys.bulletinBoard, []);
  return adminPreviewMode
    ? posts
    : posts.filter((post) => !isAdminPreviewBulletin(post));
}
...
async function replyBulletin(postId, body, accentColor) {
  const data = await apiRequest("/api/bulletin", {
    method: "POST",
    body: {
      action: "reply",
      ...
      body,
      accentColor,
      language: preferredBulletinLanguage(getProfile()),
    },
  });
  if (data?.ok && Array.isArray(data.posts)) saveBulletinPosts(data.posts);
  renderBulletinBoard();
...
async function reactBulletin(postId) {
  const data = await apiRequest("/api/bulletin", {
    method: "POST",
    body: { action: "react", id: postId },
  });
  if (data?.ok && Array.isArray(data.posts)) saveBulletinPosts(data.posts);
  renderBulletinBoard();
  return data?.post || null;
}
const bulletinAccentColors = new Set([
  "#1d4ed8",
  "#0f766e",
  "#7c3aed",
  "#b42318",
...
function bulletinAccent(value = "") {
  const color = String(value || "").trim().toLowerCase();
  return bulletinAccentColors.has(color) ? color : "#1d4ed8";
}
function safeCommunityLogoUrl(value = "") {
...
function bulletinAvatarMarkup(person = {}) {
  const logo = safeCommunityLogoUrl(
    person.authorLogoUrl || person.logoUrl || person.avatarUrl,
  );
  if (logo)
    return `<img src="${escapeHtml(logo)}" alt="${escapeHtml(person.authorName || "Member")} logo" loading="lazy" />`;
  const avatar =
    (person.authorName || "Community").trim().charAt(0).toUpperCase() || "•";
  return escapeHtml(avatar);
}
function communityHandle(value = "") {
  const handle = String(value || "member")
    .replace(/[^a-z0-9]+/gi, "")
    .slice(0, 24);
  return `@${handle || "member"}`;
}
function formatCommunityText(value = "") {
  return escapeHtml(value).replace(
    /(^|[\s(])(@[a-z0-9_][a-z0-9_.-]{1,40})/gi,
    '$1<span class="community-mention">$2</span>',
  );
}
function communityCompanyCatalog() {
  const companyLoads = adminPreviewMode ? loadSeed : loadCatalogItems();
  const fromLoads = companyLoads.map((load) => load.broker).filter(Boolean);
  const fromPosts = bulletinPosts().map((post) => post.authorName).filter(Boolean);
  return [...new Set([...fromLoads, ...fromPosts])].slice(0, 7);
}
function appendCommunityMention(company) {
  const field = document.querySelector('#bulletinForm [name="body"]');
  if (!field) return;
  const handle = communityHandle(company);
  const spacer = field.value && !/\s$/.test(field.value) ? " " : "";
  field.value = `${field.value}${spacer}${handle} `;
  field.focus();
}
function renderCommunityChrome(profile = getProfile()) {
  const displayName = profile.company || profile.name || "Your workspace";
  const initials = displayName
    ...
    .slice(0, 2)
    .toUpperCase();
  ["communityMemberAvatar", "communityComposerAvatar"].forEach((id) => {
    const el = document.getElementById(id);
    if (el) el.textContent = initials || "RM";
  });
  const name = document.getElementById("communityMemberName");
  const company = document.getElementById("communityMemberCompany");
  if (name) name.textContent = displayName;
  if (company) company.textContent = profileRoleLabel(profile.type);
  const companies = communityCompanyCatalog();
  const suggestions = document.getElementById("communityMentionSuggestions");
  if (suggestions) {
    suggestions.innerHTML = companies
      .slice(0, 4)
      .map(
        (item) =>
          `<button type="button" data-community-mention="${escapeHtml(item)}">${escapeHtml(communityHandle(item))}</button>`,
      )
      .join("");
  }
  const list = document.getElementById("communityCompanyList");
  if (list) {
    list.innerHTML = companies
      .map((item) => {
        const initials = item
          ...
          .slice(0, 2)
          .toUpperCase();
        return `<div class="community-company"><span class="community-company-avatar">${escapeHtml(initials)}</span><div><strong>${escapeHtml(item)}</strong><span>${escapeHtml(communityHandle(item))}</span></div><button type="button" data-community-mention="${escapeHtml(item)}">Tag</button></div>`;
      })
      .join("");
  }
  $$("[data-community-mention]").forEach(
    (button) =>
      (button.onclick = () => appendCommunityMention(button.dataset.communityMention)),
  );
}
...
function renderBulletinBoard() {
        ...
        .map((post) => {
          ...
          const visibleBlock =
            targetLang === sourceLang || !translatedText
              ? `<p class="bulletin-text">${formatCommunityText(post.body || "")}</p>`
          ...
          const translateBtn =
            targetLang !== sourceLang && !translatedText
              ...
              : "";
          const accent = bulletinAccent(post.accentColor);
          const replies = (Array.isArray(post.replies) ? post.replies : [])
            .map((reply) => {
              const replyAccent = bulletinAccent(reply.accentColor);
              return `<div class="bulletin-reply" style="--reply-accent:${escapeHtml(replyAccent)}"><div class="bulletin-avatar">${bulletinAvatarMarkup(reply)}</div><div class="bulletin-reply-bubble"><strong>${escapeHtml(reply.authorName || "Member")}</strong><small class="muted"> ${escapeHtml(communityHandle(reply.authorName))} · ${escapeHtml(reply.authorRole || "Member")} · ${reply.createdAt ? new Date(reply.createdAt).toLocaleString() : ""}</small><p>${formatCommunityText(reply.body || "")}</p></div></div>`;
            })
            .join("");
          const liked = (post.likedBy || []).includes(profile.userId);
          const saved = readJSON(storageKeys.savedBulletins, []).includes(post.id);
          return `<article class="card bulletin-card" style="--post-accent:${escapeHtml(accent)}"><div class="bulletin-thread"><div class="bulletin-avatar">${bulletinAvatarMarkup(post)}</div><div class="bulletin-bubble"><div class="bulletin-meta"><div><h3>${escapeHtml(post.subject || "Community post")}</h3><p class="muted"><strong>${escapeHtml(post.authorName || "Community")}</strong> <span class="bulletin-handle">${escapeHtml(communityHandle(post.authorName))}</span> · ${escapeHtml(post.authorRole || "Member")} · ${post.createdAt ? new Date(post.createdAt).toLocaleString() : ""}</p></div><span class="tag">${bulletinLanguageLabel(post.language || "en")}</span></div>${visibleBlock}${translateBtn}<div class="bulletin-social-actions"><div class="bulletin-social-buttons"><button class="btn btn-soft ${liked ? "active" : ""}" type="button" data-bulletin-like="${escapeHtml(post.id)}">♥ ${Array.isArray(post.likedBy) ? post.likedBy.length : 0}</button><button class="btn btn-soft" type="button" data-bulletin-reply-toggle="${escapeHtml(post.id)}">Reply</button><button class="btn btn-soft ${saved ? "active" : ""}" type="button" data-bulletin-save="${escapeHtml(post.id)}">${saved ? "Saved" : "Save"}</button></div><span class="muted">${Array.isArray(post.replies) ? post.replies.length : 0} ${Array.isArray(post.replies) && post.replies.length === 1 ? "reply" : "replies"}</span></div><form class="bulletin-reply-form" data-bulletin-reply-form="${escapeHtml(post.id)}"><input name="reply" maxlength="800" required placeholder="Reply to ${escapeHtml(communityHandle(post.authorName))}…" aria-label="Reply to ${escapeHtml(post.subject || "community post")}" /><button class="btn btn-primary" type="submit">Post reply</button></form></div></div>${replies ? `<div class="bulletin-replies">${replies}</div>` : ""}</article>`;
        })
        .join("")
  ...
  $$("[data-bulletin-like]").forEach(
    (button) =>
      (button.onclick = () =>
        reactBulletin(button.dataset.bulletinLike).catch((err) =>
          toast(err?.data?.error || "Like could not be saved."),
        )),
  );
  $$("[data-bulletin-save]").forEach((button) => {
    button.onclick = () => {
      const saved = new Set(readJSON(storageKeys.savedBulletins, []));
      const id = button.dataset.bulletinSave;
      saved.has(id) ? saved.delete(id) : saved.add(id);
      writeJSON(storageKeys.savedBulletins, [...saved]);
      renderBulletinBoard();
    };
  });
  $$('[data-bulletin-reply-toggle]').forEach((button) => {
    button.onclick = () => {
      const form = $$('[data-bulletin-reply-form]').find(
        (candidate) =>
          candidate.dataset.bulletinReplyForm ===
          button.dataset.bulletinReplyToggle,
      );
      if (!form) return;
  ...
  $$('[data-bulletin-reply-form]').forEach((form) => {
    form.onsubmit = async (event) => {
      ...
      try {
        await replyBulletin(
          form.dataset.bulletinReplyForm,
          body,
          getProfile().bulletinColor,
        );
        toast("Reply posted.");
      } catch (err) {
        if (submit) submit.disabled = false;
        toast(err?.data?.error || "Reply could not be posted.");
      }
    };
  });
}
...
function communicationMentionMenuMarkup(query = "") {
  ...
  return suggestions
    .map(
      (person) =>
        `<button class="comm-item comm-mention" type="button" data-communication-mention="${escapeHtml(person.handle || communicationHandle(person))}" data-mention-user="${escapeHtml(person.userId)}"><div class="comm-avatar-wrap">${communicationAvatarMarkup(person)}</div><strong>${escapeHtml(communicationDisplayName(person))}</strong><span>${escapeHtml([person.company, person.role, communicationLocationLabel(person)].filter(Boolean).join(" · ") || "Member")}</span><small>${escapeHtml(person.verificationBadge || "Membership status not supplied")} · ${escapeHtml(person.handle || communicationHandle(person))}</small></button>`,
    )
    .join("");
}
...
function communicationMemberMarkup(member) {
  const contact = [
    ...
    .filter(Boolean)
    .join(" · ");
  return `<article class="mini-item"><strong>${escapeHtml(member.name || "Member")}</strong><span>${escapeHtml([member.company, member.role, member.city, member.state].filter(Boolean).join(" · ") || "Member")}</span><small>${escapeHtml(member.verificationBadge || "Membership status not supplied")} · ${escapeHtml(member.handle || "@member")}${contact ? ` · ${escapeHtml(contact)}` : ""}</small></article>`;
}
  ...
  if (contactBox)
    contactBox.innerHTML = communicationState.contacts.length
      ? communicationState.contacts
          .map(
            (person) =>
              `<button class="comm-item" type="button" data-communication-contact="${escapeHtml(person.userId)}" data-mention-handle="${escapeHtml(person.handle || communicationHandle(person))}"><div class="comm-row"><div class="comm-avatar-wrap">${communicationAvatarMarkup(person)}</div><div><strong>${escapeHtml(communicationDisplayName(person))}</strong><span>${escapeHtml([person.company, person.role, communicationLocationLabel(person)].filter(Boolean).join(" · ") || "Member")}</span><small>${escapeHtml(person.verificationBadge || "Membership status not supplied")} · ${escapeHtml(person.handle || communicationHandle(person))}</small></div></div></button>`,
          )
          .join("")
      : '<div class="empty-state"><h4>Search members</h4><p>Search by name, company, role, location, or equipment type.</p></div>';
  if (notificationBox)
...
function route(id) {
  ...
  if (id === "dashboard" || id === "bulletin-form") id = "bulletin";
  ...
  if (!ownerPreviewActive && !publicRoutes.has(id) && id !== "billing") {
  ...
  if (
    ...
    id = "post";
  if (id === "bulletin" && !canPostBulletin(profile)) id = "pricing";
  if (id === "workbench") renderMemberWorkbench();
  $$(".screen").forEach((s) => s.classList.toggle("active", s.id === id));
  $$("[data-route]").forEach((a) =>
    a.classList.toggle(
      "active",
      a.dataset.route === id || (id === "bulletin" && a.dataset.route === "dashboard"),
    ),
  );
  location.hash = id;
...
function configureSignupForm() {
  ...
  const title = $("#signupTitle");
  const intro = $("#signupIntro");
  const eyebrow = $("#signupEyebrow");
  ...
  if (completionMode) {
    ...
    for (const field of [
      ...
      ["type", profile.type],
      ["mc_dot", profile.mc_dot],
      ["truck_count", profile.truckCount],
      ["preferredLanguage", profile.preferredLanguage],
    ]) {
      const input = form.querySelector(`[name="${field[0]}"]`);
      if (input && field[1] !== undefined && field[1] !== null)
        input.value = String(field[1]);
    }
    if (submit) submit.textContent = "Save company details & continue";
    return;
  }

  if (eyebrow) eyebrow.textContent = "Create your workspace";
  if (title) title.textContent = "Create your account once";
  if (intro)
    intro.textContent =
      "Enter one password, add your company details, and stay signed in through verification and payment.";
  if (steps)
    steps.innerHTML =
...
function loadCard(
  ...
  const confirmedAt = Date.parse(l.lastConfirmedAt || l.confirmedAt || "");
  const expiresAt = Date.parse(l.expiresAt || "");
  const lastConfirmedLabel = Number.isFinite(confirmedAt)
  ...
  const verificationLabel =
    ...
    l.tags.some((tag) => /verified/i.test(String(tag)))
      ? "Verified details supplied"
      : "Verification not supplied");
  const paymentTermsLabel =
    l.paymentTerms || l.pay || "Payment terms not provided";
  const siteReadinessLabel =
    [
      typeof l.dock === "boolean" ? `Dock: ${l.dock ? "Yes" : "No"}` : "",
      typeof l.forklift === "boolean"
        ? `Forklift: ${l.forklift ? "Yes" : "No"}`
        : "",
      l.lift ? "Liftgate" : "",
      l.ramp ? "Ramp" : "",
    ]
      .filter(Boolean)
      .join(" · ") || "Site access not provided";
  ...
  const posterLogo = safeCommunityLogoUrl(l.postedByLogoUrl);
  const posterIcon = posterLogo
    ? `<img class="load-poster-logo" src="${escapeHtml(posterLogo)}" alt="" loading="lazy" referrerpolicy="no-referrer" />`
    : "";
  const photoGallery = Array.isArray(l.photos) && l.photos.length
    ? `<div class="load-photo-gallery" aria-label="Freight photos">${l.photos
        .map(
          (photo) =>
            `<img src="/api/loads?loadId=${encodeURIComponent(l.id)}&amp;photoId=${encodeURIComponent(photo.id)}" alt="${escapeHtml(photo.alt || "Freight condition photo")}" loading="lazy" />`,
        )
        .join("")}</div>`
    : "";
...
function renderLoads() {
  ...
  if (!loadCatalogLoaded) {
    if (box)
      box.innerHTML =
        '<div class="card"><h3>Loading load board…</h3><p class="muted">Fetching load data from the server.</p></div>';
  ...
  if (loadCatalogError) {
    if (box)
      box.innerHTML =
        `<div class="card empty-state"><h3>Load board unavailable</h3><p>${escapeHtml(loadCatalogError)}</p><button class="btn btn-soft" type="button" data-route="workbench">Return to Workbench</button></div>`;
  ...
  $$("[data-report-load]").forEach((button) => {
    button.onclick = async () => {
      const reason = prompt(
        "Why are you reporting this load?\n(Examples: suspected fraud, duplicate post, wrong details, spam)",
      );
      if (!reason || !reason.trim()) return;
      try {
        const result = await apiRequest("/api/loads", {
          method: "POST",
          body: {
            action: "report",
            loadId: String(button.dataset.reportLoad || ""),
            reason: reason.trim().slice(0, 200),
          },
        });
        toast(
          result?.underReview
            ? "Report received. This load is now under review and hidden from the board."
            : "Report received. The review team will look at this load.",
        );
        if (result?.underReview) await loadLoadCatalog(true).catch(() => {});
      } catch (err) {
        toast(err?.data?.error || "Could not send the report. Try again.");
      }
    };
  });
  $$("[data-book]").forEach((button) => {
    button.onclick = () => {
      ...
      if (currentStatus.mode === "blocked") {
        toast("Your plan cannot bid on loads.");
        return;
      }
      ...
      const form = box?.querySelector(`[data-bid-form="${CSS.escape(load.id)}"]`);
      if (form) {
        form.hidden = false;
        const amount = form.querySelector('[name="bid_amount"]');
        if (amount) amount.focus();
      }
    };
  });
  $$("[data-cancel-bid]").forEach((button) => {
    button.onclick = () => {
      const form = box?.querySelector(
        `[data-bid-form="${CSS.escape(String(button.dataset.cancelBid || ""))}"]`,
      );
      if (form) form.hidden = true;
    };
  });
  $$("[data-bid-form]").forEach((form) => {
    form.onsubmit = async (event) => {
      event.preventDefault();
      const load = catalog.find((item) => item.id === form.dataset.bidForm);
      if (!load) return toast("That load is no longer available.");
      const amount = Number(form.querySelector('[name="bid_amount"]')?.value || 0);
      ...
      try {
        await apiRequest("/api/loads", {
          method: "POST",
          body: { action: "bid", loadId: load.id, amount, note },
        });
        await loadLoadCatalog(true);
        pushRecent(storageKeys.recentLoads, {
          title: `${load.from} → ${load.to}`,
          detail: `Bid submitted · ${money(amount)} · ${load.eq} · ${load.pick}`,
        });
        toast(`Bid sent: ${money(amount)} for ${load.from} → ${load.to}`);
        renderLoads();
        renderProfile();
      } catch (error) {
        if (submit) submit.disabled = false;
        toast(error?.data?.error || error?.message || "Could not submit this bid.");
      }
    };
  });
  renderSavedLoadSearches();
}
function memberWorkbenchDate(value = "") {
...
function memberWorkbenchTrustValue(value, fallback = "Not provided") {
...
function renderMemberWorkbench() {
  const profile = getProfile();
  const shipperOnly = isRequestOnlyAccount(profile);
  const carrierActions = $("#workbenchCarrierActions");
  const shipperActions = $("#workbenchShipperActions");
  const carrierPanel = $("#workbenchCarrierPanel");
  const shipperPanel = $("#workbenchShipperPanel");
  if (!carrierPanel || !shipperPanel) return;

  carrierActions.hidden = shipperOnly;
  ...
  const planLabel = shipperOnly
    ...
    : profile.planLabel ||
      ...
      "Carrier member";
  const accountName =
    profile.company || profile.name || (shipperOnly ? "Shipper workspace" : "Carrier workspace");
  ...
  const savedSearches = readJSON(storageKeys.savedLoadSearches, []);
  const messages = readJSON(storageKeys.messages, []);
  const recentRequests = readJSON(storageKeys.recentRequests, []);
  const history = readJSON(storageKeys.loadHistory, []);
  const postedHistory = history.filter((item) => item.eventType === "posted");
  ...
  $("#workbenchMetricTwoLabel").textContent = shipperOnly
    ? "Carrier responses"
    : "Saved lanes";
  $("#workbenchMetricTwo").textContent = shipperOnly
    ? String(messages.length)
    : String(laneAlerts.length + savedSearches.length);
  $("#workbenchMetricTwoHelp").textContent = shipperOnly
    ? "Message threads"
    : "Alerts and saved searches";
  $("#workbenchMessageCount").textContent = String(messages.length);

  const bookingReady = !shipperOnly && bookingVerificationReady(profile);
  const emailVerified = Boolean(
    String(
      profile.emailVerifiedAt || profile.memberAccess?.emailVerifiedAt || "",
    ).trim() || profile.memberAccess?.emailVerified,
  );
  const profileComplete = isProfileCompleteState(profile);
  const dotValue = shipperOnly
    ? "Not required for shipper plan"
    : memberWorkbenchTrustValue(profile.dotMcStatus || profile.mc_dot);
  const insuranceValue = shipperOnly
    ? "Not required for shipper plan"
    : memberWorkbenchTrustValue(profile.insuranceStatus);
  const trustLabel = shipperOnly
    ? emailVerified && profileComplete
      ? "Posting ready"
      : "Action needed"
    : bookingReady
      ? "Ready"
      : emailVerified && profileComplete
        ? "Review"
        : "Action needed";
  $("#workbenchTrustStatus").textContent = trustLabel;
  $("#workbenchTrustHelp").textContent = shipperOnly
    ? "Posting access only — load claiming is not included"
    : bookingReady
      ? "Booking checks currently satisfied"
      : "Open Profile to finish required checks";
  $("#workbenchTrustTag").textContent = trustLabel;
  $("#workbenchEmailTrust").textContent = emailVerified ? "Verified" : "Not verified";
  $("#workbenchDotTrust").textContent = dotValue;
  $("#workbenchInsuranceTrust").textContent = insuranceValue;
  ...
  carrierLoads.innerHTML = loadCatalogError
    ? `<div class="empty-state"><h4>Load board unavailable</h4><p>${escapeHtml(loadCatalogError)}</p></div>`
    : catalog.length
    ? catalog
        .map((load) => {
          const lastConfirmed = load.lastConfirmedAt || load.confirmedAt || "";
          const verification =
            load.verificationStatus ||
            (Array.isArray(load.tags) &&
            load.tags.some((tag) => /verified/i.test(String(tag)))
              ? "Verified details supplied"
              : "Verification not supplied");
          return `<a class="workbench-row workbench-load-row" href="#loads"><div><strong>${escapeHtml(load.from)} → ${escapeHtml(load.to)}</strong><span>${escapeHtml(load.eq || "Equipment not provided")} · ${escapeHtml(load.pick || "Pickup time not provided")} · ${money(load.rate)}</span></div><small>${escapeHtml(verification)} · ${escapeHtml(lastConfirmed ? `Confirmed ${memberWorkbenchDate(lastConfirmed)}` : "Not recently confirmed")}</small></a>`;
        })
        .join("")
    : '<div class="empty-state"><h4>No open loads yet</h4><p>Save a lane alert. Real postings will appear here without demo inventory.</p></div>';

  const shipperLoads = $("#workbenchShipperLoads");
  const shipperItems = [...recentRequests, ...postedHistory].slice(0, 4);
  shipperLoads.innerHTML = shipperItems.length
    ? shipperItems
        .map((item) => {
          const title =
            item.title ||
          ...
          const detail =
            item.detail ||
            [item.equipment, item.status].filter(Boolean).join(" · ") ||
            "Waiting for complete posting details.";
          return `<article class="workbench-row"><div><strong>${escapeHtml(title)}</strong><span>${escapeHtml(detail)}</span></div><small>${escapeHtml(item.status || "Posted")}</small></article>`;
        })
        .join("")
    : '<div class="empty-state"><h4>No pickups posted yet</h4><p>Use Post a pickup. Required equipment, dimensions, access, timing, and payment details help carriers evaluate the job.</p></div>';
}
...
function renderBrokerTrustLedger() {
  const ledger = $("#trustLedger");
  if (!ledger) return;
  ledger.innerHTML = adminPreviewMode
    ? brokerTrust
        .map(
          (b) =>
        ...
        .join("")
    : '<div class="card empty-state"><h3>No broker trust records available</h3><p class="muted">Only server-backed broker records will appear here.</p></div>';
}
...
function profileHistoryEntries() {
  const serverHistory = readJSON(storageKeys.loadHistory, []);
  const completed = verifiedTransactions().map((item) => ({
    id: item.id || `completed:${item.loadId || item.completedAt}`,
    loadId: item.loadId || "",
    eventType: "completed",
    status: "completed",
    title:
    ...
    detail: item.detail || "Delivery completed",
    occurredAt: item.completedAt || item.createdAt || item.savedAt || "",
    verified: item.verified !== false,
  }));
  const reviews = trustEntries().map((item) => ({
    id: `review:${item.id || item.createdAt}`,
    loadId: item.reviewTransactionId || "",
    eventType: "review",
    status: "reviewed",
    title: `Review: ${item.name || item.kind || "Member"}`,
    detail:
      item.reviewQuestionSummary ||
      item.notes ||
      `${Number(item.score || 0)}/100 trust review`,
    occurredAt: item.createdAt || item.verifiedCompletedAt || "",
    verified: Boolean(item.reviewTransactionId || item.verifiedCompletedAt),
  }));
  const activePickups = isRequestOnlyAccount(getProfile())
    ? []
    : readJSON(storageKeys.activePickups, []).filter(
        (item) => item && item.serverAuthorized === true,
      );
  const activeEvents = activePickups.flatMap((item) =>
    (Array.isArray(item.statusHistory) ? item.statusHistory : []).map((step) => {
      const normalized = String(step.status || "").toLowerCase();
      const eventType = normalized.includes("confirm")
        ? "accepted"
        : normalized.includes("picked")
          ...
          : normalized.includes("transit")
            ...
            : normalized.includes("deliver")
              ...
              : "pickup_requested";
      return {
        id: `pickup:${item.id || item.loadId}:${eventType}:${step.at || ""}`,
        loadId: item.loadId || item.id || "",
        eventType,
        status: normalized || eventType,
        title:
          item.title ||
          [item.origin, item.destination].filter(Boolean).join(" → ") ||
          "Pickup activity",
        ...
        destination: item.destination || "",
        equipment: item.equipment || "",
        detail: item.detail || step.status || "Pickup activity",
        occurredAt: step.at || item.savedAt || "",
        verified: false,
      };
    }),
  );
  const byId = new Map();
  [...serverHistory, ...completed, ...reviews, ...activeEvents].forEach((item) => {
    const key = String(
      item.id ||
        `${item.loadId || item.title}:${item.eventType}:${item.occurredAt || ""}`,
    );
    if (!byId.has(key)) byId.set(key, item);
  });
  return [...byId.values()].sort(
    (a, b) =>
      (Date.parse(b.occurredAt || 0) || 0) -
      (Date.parse(a.occurredAt || 0) || 0),
  );
}
...
function renderProfileHistory() {
  ...
  const summary = $("#profileHistorySummary");
  const filter = $("#profileHistoryFilter");
  if (!list || !summary) return;
  const entries = profileHistoryEntries();
  const count = (types) =>
    entries.filter((item) => types.includes(item.eventType)).length;
  const metrics = [
    ["Loads posted", count(["posted"])],
    ["Loads viewed", count(["viewed"])],
    ["Responses", count(["pickup_requested", "bid_submitted", "bid_declined", "accepted"])],
  ...
  const shown = entries.filter((item) => {
    ...
    if (selected === "posted") return item.eventType === "posted";
    if (selected === "viewed") return item.eventType === "viewed";
    if (selected === "pickup")
      return ["pickup_requested", "bid_submitted", "bid_declined", "accepted", "picked_up", "in_transit"].includes(
        item.eventType,
      );
    if (selected === "completed") return item.eventType === "completed";
    return item.eventType === "review";
  });
    ...
    ? shown
        .map((item) => {
          const lane =
            ...
            item.title ||
            "Load activity";
          const detail = [item.equipment, item.counterpartyName, item.detail]
            .filter(Boolean)
            .join(" · ");
          const when = item.occurredAt
            ? new Date(item.occurredAt).toLocaleString()
            : "Date unavailable";
          return `<article class="mini-item history-event ${escapeHtml(item.eventType || "")}"><div class="section-head"><strong>${escapeHtml(lane)}</strong><span class="tag ${item.verified ? "good" : ""}">${escapeHtml(historyEventLabel(item.eventType))}</span></div><span>${escapeHtml(detail || historyEventLabel(item.eventType))}</span><small>${escapeHtml(when)} · ${item.verified ? "Verified activity" : "Account record"}</small></article>`;
        })
        .join("")
    : '<div class="empty-state"><h4>No matching activity yet</h4><p>Posts, pickup responses, completed work, and reviews will build this timeline automatically.</p></div>';
  if (filter && !filter.dataset.bound) {
    filter.dataset.bound = "true";
    filter.addEventListener("change", renderProfileHistory);
  }
}
function renderPostedLoadResponses() {
  const list = $("#profilePostedResponses");
  const count = $("#profileResponseCount");
  if (!list || !count) return;
  const responses = postedLoadCatalog.flatMap((load) =>
    (Array.isArray(load.claimRequests) ? load.claimRequests : []).map(
  ...
  list.innerHTML = responses.length
    ? responses
        .map(({ load, request }) => {
          ...
          return `<article class="mini-item"><div class="section-head"><div><strong>${escapeHtml(lane)}</strong><span>${escapeHtml(member)} · ${escapeHtml(profileDisplayDate(request.updatedAt || request.requestedAt, true))}</span></div><span class="tag ${accepted ? "good" : ""}">${escapeHtml(status)}</span></div><div class="bid-review-terms"><strong>${money(request.amount || load.rate)}</strong><span>${escapeHtml(request.note || "No bid note supplied")}</span><small>Shipper target: ${money(load.rate)}</small></div>${action}</article>`;
        })
        .join("")
    : '<div class="empty-state"><h4>No pickup responses yet</h4><p>Authenticated carrier requests on your posted loads will appear here. Other members cannot read this list.</p></div>';
  list.querySelectorAll("[data-accept-load]").forEach((button) => {
    button.onclick = async () => {
      const loadId = button.dataset.acceptLoad;
      const relatedButtons = [...list.querySelectorAll("[data-accept-load], [data-decline-load]")]
        .filter((candidate) =>
          candidate.dataset.acceptLoad === loadId || candidate.dataset.declineLoad === loadId,
        );
      try {
        ...
        toast(error?.data?.error || error?.message || "Request could not be accepted.");
      }
    };
  });
  list.querySelectorAll("[data-decline-load]").forEach((button) => {
    button.onclick = async () => {
      try {
        ...
        toast(error?.data?.error || error?.message || "Bid could not be declined.");
      }
    };
  });
}
function renderCancellationControl(profile) {
  ...
  const subscriptionStatus = String(profile.subscriptionStatus || "unpaid").toLowerCase();
  const endDate = profile.subscriptionCurrentPeriodEnd
  ...
  if (profile.subscriptionCancelAtPeriodEnd) {
    status.textContent = `Cancellation scheduled. Member access remains active until ${endDate}, then ends automatically.`;
  } else if (["canceled", "cancelled", "unpaid"].includes(subscriptionStatus)) {
    status.textContent =
      subscriptionStatus === "unpaid"
        ? "There is no active paid subscription on this account."
        : "This subscription is canceled and member-only access has ended.";
  } else {
    status.textContent =
      "Stripe will show your cancellation date before you confirm. Access ends automatically when the cancellation takes effect.";
  }
  button.disabled = !hasProfileIdentity(profile) || subscriptionStatus === "unpaid";
  button.onclick = async () => {
    if (
      !window.confirm(
        "Open Stripe's secure billing portal to review and confirm subscription cancellation?",
      )
    )
      return;
    try {
      button.disabled = true;
      toast("Opening secure cancellation controls…");
      await openBillingPortal();
    } catch (err) {
      toast(err?.data?.error || "Could not open cancellation controls.");
      button.disabled = false;
    }
  };
}
function profileDisplayDate(value, includeTime = false) {
...
function renderProfile() {
  ...
  const activePickups = isRequestOnlyAccount(profile)
    ? []
    : readJSON(storageKeys.activePickups, []).filter(
        (item) => item && item.serverAuthorized === true,
      );
  const profileHistoryEvents = readJSON(storageKeys.loadHistory, []);
  const paymentLabel =
  ...
  if (profileBrandIcon) {
    const logo = safeCommunityLogoUrl(profile.logoUrl);
    profileBrandIcon.innerHTML = logo
      ? `<img src="${escapeHtml(logo)}" alt="${escapeHtml(profile.company || profile.name || "Account")} logo" />`
      : escapeHtml(
          String(profile.company || profile.name || "RM")
            .split(/\s+/)
  ...
  const profileBrandForm = $("#profileBrandForm");
  if (profileBrandForm) {
    const logoField = profileBrandForm.querySelector('[name="logoUrl"]');
    const colorField = profileBrandForm.querySelector('[name="bulletinColor"]');
    if (logoField) logoField.value = profile.logoUrl || "";
    if (colorField) colorField.value = bulletinAccent(profile.bulletinColor);
  }
  ...
  if ($("#profileDotMc")) $("#profileDotMc").textContent = profile.mc_dot || "Not added";
  if ($("#profileInsuranceStatus"))
    $("#profileInsuranceStatus").textContent =
      profile.insuranceStatus || "Not submitted";
  if ($("#profileCredentialStatus"))
    $("#profileCredentialStatus").textContent =
      profile.insuranceStatus || "Not submitted";
  const profileStats = {
    profileStatLoads: profileHistoryEvents.filter(
      (item) => item?.eventType === "posted",
    ).length,
    profileStatViews: profileHistoryEvents.filter(
      (item) => item?.eventType === "viewed",
    ).length,
    profileStatPickups: activePickups.length,
    profileStatHistory: profileHistoryEvents.length,
  };
  Object.entries(profileStats).forEach(([id, value]) => {
    const node = document.getElementById(id);
    if (node) node.textContent = String(value);
  });
  const credentialsForm = $("#profileCredentialsForm");
  if (credentialsForm) {
    const values = {
      mc_dot: profile.mc_dot || "",
      insuranceProvider: profile.insuranceProvider || "",
    ...
    Object.entries(values).forEach(([name, value]) => {
      const field = credentialsForm.querySelector(`[name="${name}"]`);
  ...
  if (postForm) {
    ...
    if (notice)
      notice.textContent = allowed
        ? "Add complete handling details and optional freight photos to prevent surprise labor issues."
...
const checkoutPlans = {
  ...
  "fleet-starter": {
    label: "Carrier & Broker 1–3 Trucks",
    price: "$59.99/mo",
    href: "https://buy.stripe.com/3cI4gA8Nc6AH6UW6WYaVa02",
    copy: "For small carriers, fleets, and brokers covering early demand.",
    tags: ["View demand", "Claim loads", "Post covered freight"],
    cta: "Continue to checkout",
  },
  "fleet-growth": {
    label: "Carrier & Broker 4–7 Trucks",
    price: "$79.99/mo",
    href: "https://buy.stripe.com/00w00kfbAcZ52EG5SUaVa04",
    copy: "For growing teams with more truck coverage.",
    tags: ["Post loads", "Coverage for 4–7 trucks", "Saved lanes"],
    cta: "Continue to checkout",
  },
  "fleet-pro": {
    label: "Carrier & Broker 8–12 Trucks",
    price: "$149.99/mo",
    href: "https://buy.stripe.com/3cIcN69RgaQXbbcftuaVa03",
    copy: "For higher-capacity teams covering more lanes.",
    tags: ["Post loads", "Coverage for 8–12 trucks", "Priority placement"],
    cta: "Continue to checkout",
  },
  "dispatcher-broker": {
    label: "Dispatcher & Broker",
    price: "$189.99/mo",
    href: "https://buy.stripe.com/fZu28s1kK0cj5QSgxyaVa05",
    copy: "For dispatchers and brokers coordinating freight, carrier coverage, and customer communication.",
    tags: ["Find, request, and post loads", "Dispatch workspace", "Broker tools"],
    cta: "Continue to checkout",
  },
};
...
function initPlanChooser() {
        ...
        planKey === "driver"
          ...
          : planKey === "fleet-starter"
            ...
            : planKey === "fleet-growth"
              ...
              : planKey === "fleet-pro"
                ? "Broker 8–12 trucks - $149.99/mo"
                : planKey === "dispatcher-broker"
                  ? "Dispatcher / broker - $189.99/mo"
  ...
  $$('[data-home-plan]').forEach((link) => {
    link.addEventListener("click", (event) => {
      event.preventDefault();
      setPlan(link.dataset.homePlan || "shipper");
      route("pricing");
    });
  });
...
function submittedDirectRequests() {
  const saved = readJSON(storageKeys.directRequests, []);
  return Array.isArray(saved) ? saved : [];
}
function allDirectRequests() {
  return adminPreviewMode
    ? [...submittedDirectRequests(), ...directRequests]
    : [];
}
function saveSubmittedDirectRequest(request) {
  const current = submittedDirectRequests();
  writeJSON(storageKeys.directRequests, [request, ...current].slice(0, 30));
  scheduleAccountSync();
}
function placeRequestBid(req) {
  const profile = getProfile();
  const entered = prompt(
    `Counteroffer for ${req.from} → ${req.to}: what is your all-in price?`,
    req.budget.replace(/[^\d.]/g, ""),
  );
  if (entered === null) return;
  const amount = Number(String(entered).replace(/[^\d.]/g, ""));
  if (!Number.isFinite(amount) || amount <= 0) {
    toast("Enter a real offer amount.");
    return;
  }
  const note = prompt(
    "What does this price include? (equipment, loading help, timing, tolls, or questions)",
    "",
  ) || "";
  const bid = {
    id: crypto.randomUUID(),
  ...
  toast(`Counteroffer saved: ${money(amount)} on ${req.from} → ${req.to}`);
  renderRequests();
  renderProfile();
}
function acceptRequestBid(req, bid) {
  ...
  saveRequestBidStore(store);
  toast(`Rate and terms accepted at ${money(bid.amount)} for ${req.from} → ${req.to}`);
  renderRequests();
  renderProfile();
}
function renderRequests() {
  ...
  if (!box) return;
  box.innerHTML = allDirectRequests()
    .map((r, index) => {
      ...
      const bidHtml = bids.length
        ? `<div class="request-bids">${bids.map((bid) => `<article class="mini-item ${bid.accepted ? "accepted" : ""}"><strong>${money(bid.amount)} · ${escapeHtml(bid.bidderName || "Driver")}</strong><span>${escapeHtml(bid.note || "Ask provider what is included")}</span><small>${bid.bidderRole || "Driver"}${bid.accepted ? " · Rate and terms accepted" : " · Counteroffer"} · ${bid.createdAt ? new Date(bid.createdAt).toLocaleString() : ""}</small><button class="btn btn-soft" data-accept-bid="${r.id}" data-bid-id="${bid.id}">${bid.accepted ? "Agreed" : "Accept rate & terms"}</button></article>`).join("")}</div>`
        : "";
      ...
      const rateTerms = [r.rateTerms, ...(r.rateItems || [])].filter(Boolean).join(" · ");
      const siteConditions = (r.siteConditions || []).join(" · ");
      return `<article class="card request-card"><div class="request-top"><div class="request-media">${media}<div><div class="request-badge">${r.submittedByMember ? "Your request" : "Direct request"}</div><div class="route">${escapeHtml(r.from)} → ${escapeHtml(r.to)}</div><p class="muted request-meta">${escapeHtml(r.customer)} · ${escapeHtml(r.when)} · ${escapeHtml(r.equipment)}</p></div></div><div class="request-rate">${escapeHtml(r.budget)}<small>shipper target</small></div></div><p class="request-copy">${escapeHtml(r.details)}</p>${rateTerms ? `<div class="agreement-summary"><strong>Rate should cover</strong><span>${escapeHtml(rateTerms)}</span></div>` : ""}${siteConditions ? `<div class="agreement-summary"><strong>Site conditions</strong><span>${escapeHtml(siteConditions)}</span></div>` : ""}<div class="chips request-chips"><span class="chip good">Needs coverage</span>${autoChip}<span class="chip warn">Confirm profile details</span><span class="chip">Carrier/fleet/broker visible</span>${latestBid ? `<span class="chip good">Latest offer ${money(latestBid.amount)}</span>` : ""}${acceptedBid ? `<span class="chip good">Agreed ${money(acceptedBid.amount)}</span>` : ""}<span class="chip">${bids.length} offer${bids.length === 1 ? "" : "s"}</span></div><div class="load-actions request-actions"><span class="muted">Choose the provider and terms that fit—not simply the lowest number.</span><button class="btn btn-soft" data-bid="${index}">Send counteroffer</button><button class="btn btn-primary" data-request="${index}">Save request</button></div>${bidHtml}</article>`;
    })
    .join("");
  $$("[data-bid]").forEach(
    (b) =>
      (b.onclick = () =>
        placeRequestBid(allDirectRequests()[Number(b.dataset.bid)])),
  );
  $$("[data-accept-bid]").forEach(
    (b) =>
      (b.onclick = () => {
        const req = allDirectRequests().find(
  ...
  $$("[data-request]").forEach(
    (b) =>
      (b.onclick = () => {
        const req = allDirectRequests()[Number(b.dataset.request)];
...
function initForms() {
  ...
  if (signup)
    signup.addEventListener("submit", async (e) => {
      e.preventDefault();
      const form = e.currentTarget;
      const completionMode = form.dataset.mode === "complete-profile";
      const email = String(
        form.querySelector('[name="email"]').value || "",
      ).trim();
      ...
      if (!email || (!completionMode && !password)) {
        toast(
          completionMode
            ? "Your signed-in email is missing. Reload and try again."
      ...
      try {
        const data = completionMode
          ? await completeProfileAccount(profile)
          : await registerAccount(profile, password);
        ...
        if (accountNotice) {
          ...
          accountNotice.textContent =
            data?.dashboardRoute === "verify" ||
            data?.accessRoute === "verify" ||
        ...
        toast(
          err?.data?.error ||
            (completionMode
              ? "Could not save your company details right now."
  ...
  if (resetForm)
    resetForm.addEventListener("submit", async (e) => {
      ...
      if (!email || !token || !password || !confirmPassword) {
        toast("Enter your reset token and both password fields.");
        return;
      }
      ...
      if (password !== confirmPassword) {
        toast("The two passwords need to match.");
        return;
      }
      try {
        const data = await apiRequest("/api/account", {
  ...
  if (bulletinForm)
    bulletinForm.addEventListener("submit", async (e) => {
      ...
      const accentColor = bulletinAccent(
        bulletinForm.querySelector('[name="accentColor"]')?.value ||
          profile.bulletinColor,
      );
      if (!body.trim()) {
      ...
      if (form)
        form.addEventListener("submit", async (e) => {
          ...
          if (id === "postForm") {
            ...
            const detailParts = [
              field("commodity")?.value || "Commodity not set",
              equipment,
              field("dimensions")?.value || "Dimensions not set",
            ...
            try {
              const posted = await apiRequest("/api/loads", {
                ...
                body: {
                  ...
                  loadingHelp: field("loading_help")?.value || "",
                  siteConditions: field("site_conditions")?.value || "",
                  contactName: field("contact_name")?.value || "",
                  contactPhone: field("contact_phone")?.value || "",
                  declaredValue: parseMoney(field("declared_value")?.value || ""),
                  dock: Boolean(field("dock_at_pickup")?.checked),
                  forklift: Boolean(field("forklift_at_pickup")?.checked),
                  autoMode,
                  directLoad,
            ...
            const field = (name) => form.querySelector(`[name="${name}"]`);
            const pickup = field("pickup_city")?.value || "Pickup";
            const delivery = field("delivery_city")?.value || "Delivery";
            const window = field("pickup_window")?.value || "Timing to confirm";
            const equipment = field("equipment_needed")?.value || "Equipment to confirm";
            const target = field("target_rate")?.value || "Open to offers";
            const terms = field("rate_terms")?.value || "Terms to confirm";
            const rateItems = Array.from(
              form.querySelectorAll('[name="rate_item"]:checked'),
            ).map((item) => item.value);
            const siteConditions = Array.from(
              form.querySelectorAll('[name="site_condition"]:checked'),
            ).map((item) => item.value);
            const data = {
              title: `${pickup} → ${delivery}`,
              detail: `${equipment} · ${window} · Target: ${target} · Includes: ${terms}`,
            };
            pushRecent(storageKeys.recentRequests, data);
            const request = {
              id: `req-${crypto.randomUUID()}`,
              customer: profile.company || profile.name || "Member shipper",
              from: pickup,
              to: delivery,
              equipment,
              budget: target,
              when: window,
              details:
                field("freight_details")?.value ||
                field("weight_dimensions")?.value ||
                "Details to confirm with the shipper.",
              rateTerms: terms,
              rateItems,
              siteConditions,
              submittedByMember: true,
            };
            saveSubmittedDirectRequest(request);
            renderProfile();
            renderRequests();
            scheduleAccountSync();
            form.reset();
          ...
          toast(
            id === "ratingForm"
              ...
              : id === "disputeForm"
                ...
                : "Request posted with the target rate and agreement checklist. Confirm profile and transaction details before accepting an offer.",
          );
        });
    },
  );
  const profileBrandForm = $("#profileBrandForm");
  if (profileBrandForm)
    profileBrandForm.addEventListener("submit", async (e) => {
      e.preventDefault();
      const rawLogo = String(
        profileBrandForm.querySelector('[name="logoUrl"]')?.value || "",
      ).trim();
      const logoUrl = safeCommunityLogoUrl(rawLogo);
      if (rawLogo && !logoUrl) {
        toast("Use a complete http:// or https:// logo image link.");
        return;
      }
      const bulletinColor = bulletinAccent(
        profileBrandForm.querySelector('[name="bulletinColor"]')?.value,
      );
      try {
        const data = await apiRequest("/api/account", {
          method: "POST",
          body: { action: "save", profile: { logoUrl, bulletinColor } },
        });
        mergeAccountState(data);
        renderProfile();
        renderBulletinBoard();
  ...
  if (profileCredentialsForm)
    profileCredentialsForm.addEventListener("submit", async (e) => {
      e.preventDefault();
      const form = e.currentTarget;
      const insuranceDocumentUrl = String(
        form.querySelector('[name="insuranceDocumentUrl"]')?.value || "",
      ).trim();
      if (insuranceDocumentUrl && !/^https:\/\//i.test(insuranceDocumentUrl)) {
        toast("Use a secure https:// link for the insurance document.");
        return;
      }
      const insurancePolicyLast4 = String(
        form.querySelector('[name="insurancePolicyLast4"]')?.value || "",
      ).replace(/\D/g, "");
      if (insurancePolicyLast4 && insurancePolicyLast4.length !== 4) {
        toast("Enter only the last 4 policy digits.");
        return;
      }
      try {
        const data = await apiRequest("/api/account", {
          ...
          body: {
            action: "save",
            profile: {
              mc_dot: String(form.querySelector('[name="mc_dot"]')?.value || "").trim(),
              insuranceProvider: String(
                form.querySelector('[name="insuranceProvider"]')?.value || "",
              ).trim(),
              insurancePolicyLast4,
              insuranceExpiration: String(
                form.querySelector('[name="insuranceExpiration"]')?.value || "",
              ).trim(),
        ...
        form.querySelector('[name="insuranceDocumentUrl"]').value = "";
        renderProfile();
        toast("DOT and insurance details saved for review.");
      } catch (err) {
        toast(err?.data?.error || "Credentials could not be saved.");
      }
    });
  $$("[data-profile-jump]").forEach((button) => {
    button.onclick = () =>
      document
        .getElementById(button.dataset.profileJump)
        ...
        saved
          .map((a) => {
            ...
            const matchHtml = matches.length
              ? `<div class="alert-matches"><strong>${matches.length} load${matches.length === 1 ? "" : "s"} matched</strong>${matches
                  .slice(0, 3)
                  .map(
                    (load) =>
                  ...
                  .join("")}</div>`
              : '<p class="muted">No loads match yet. Keep the alert active.</p>';
...
function parseDispatchMoney(value = "") {
  const number = Number(String(value || "").replace(/[^0-9.]/g, ""));
  return Number.isFinite(number) && number > 0 ? number : 0;
}
function dispatchRequestFromForm() {
  return {
    goal: $("#dispatchGoal")?.value || "find_loads",
    origin: $("#dispatchOrigin")?.value || "",
    destination: $("#dispatchDestination")?.value || "Anywhere",
    equipment: $("#dispatchEquipment")?.value || "Any",
    availableDates: $("#dispatchDates")?.value || "Any",
    maxDeadheadMiles: Number($("#dispatchDeadhead")?.value || 0),
    minimumRatePerMile: parseDispatchMoney($("#dispatchRate")?.value),
    costPerMile: parseDispatchMoney($("#dispatchCostPerMile")?.value),
    desiredMarginPercent: Number($("#dispatchMargin")?.value || 0),
    notes: $("#dispatchNotes")?.value || "",
  };
}
function renderDispatchPlan(data) {
  const welcome = $("#dispatchWelcome");
  const plan = $("#dispatchPlan");
  const copy = $("#dispatchPlanCopy");
  const matchesBox = $("#dispatchMatches");
  const approval = $("#dispatchApproval");
  const laneSummary = $("#dispatchLaneSummary");
  const roleTag = $("#dispatchRoleTag");
  if (!plan || !copy || !matchesBox) return;

  latestDispatchPlan = data;
  if (welcome) welcome.hidden = true;
  plan.hidden = false;
  copy.textContent = String(data.assistantMessage || "No plan was returned.");
  if (roleTag)
    roleTag.textContent =
      data.role === "carrier" ? "Driver / carrier plan" : "Customer / shipper plan";

  const matches = Array.isArray(data.matches) ? data.matches : [];
  matchesBox.innerHTML = matches.length
    ? `<h3>Best matches</h3>${matches
        .map(
          (load, index) =>
            `<article class="dispatch-match"><div class="dispatch-match-rank">${index + 1}</div><div><strong>${escapeHtml(load.from)} → ${escapeHtml(load.to)}</strong><p>${escapeHtml(load.eq || "Equipment not listed")} · ${money(load.rate)}${load.ratePerMile ? ` · $${Number(load.ratePerMile).toFixed(2)}/mi` : ""}</p><small>${escapeHtml((load.reasons || []).join(" · "))}</small>${
              load.economics
                ? `<small class="${load.economics.estProfit >= 0 ? "dispatch-profit" : "dispatch-loss"}">${
                    load.economics.estProfit >= 0
                      ? `Est. profit ${money(load.economics.estProfit)} vs your break-even ${money(load.economics.breakEven)} · your target rate ${money(load.economics.targetRate)}`
                      : `⚠ ${money(Math.abs(load.economics.estProfit))} BELOW your break-even of ${money(load.economics.breakEven)}`
                  }</small>`
                : ""
            }${
              (load.gaps || []).length
                ? `<small class="dispatch-gaps">Check before requesting: ${escapeHtml(load.gaps.join(" "))}</small>`
                : ""
            }${
              load.askDraft
                ? `<button class="btn btn-soft" type="button" data-dispatch-ask="${escapeHtml(load.id)}">Copy questions for poster</button>`
                : ""
            }${
              load.calculatorUrl
                ? `<a class="btn btn-soft" href="${escapeHtml(load.calculatorUrl)}" target="_blank" rel="noopener">Open in rate calculator</a>`
                : ""
            }</div><button class="btn btn-soft" type="button" data-dispatch-load="${escapeHtml(load.id)}">Review load</button></article>`,
        )
        .join("")}`
    : data.role === "carrier"
      ? '<div class="notice"><strong>No match yet.</strong> Save the suggested Lane Alert so new loads can find you.</div>'
      : "";

  if (approval) approval.hidden = !data.laneAlertDraft;
  if (laneSummary && data.laneAlertDraft) {
    const draft = data.laneAlertDraft;
    laneSummary.textContent = `${draft.origin || "Anywhere"} → ${draft.destination || "Anywhere"} · ${draft.equipment || "Any equipment"}${draft.rate ? ` · $${Number(draft.rate).toFixed(2)}/mi target` : ""}`;
  }
  $$('[data-dispatch-ask]').forEach((button) => {
    button.onclick = async () => {
      const selected = matches.find(
        (load) => String(load.id) === String(button.dataset.dispatchAsk || ""),
      );
      if (!selected?.askDraft) return;
      try {
        await navigator.clipboard.writeText(selected.askDraft);
        toast("Questions copied. Paste them in a message to the poster — nothing is sent automatically.");
      } catch {
        prompt("Copy these questions:", selected.askDraft);
      }
    };
  });
  $$('[data-dispatch-load]').forEach((button) => {
    button.onclick = () => {
      const selected = matches.find(
        (load) => String(load.id) === String(button.dataset.dispatchLoad || ""),
      );
      if ($("#pickupFilter")) $("#pickupFilter").value = selected?.from || "";
      if ($("#deliveryFilter"))
        $("#deliveryFilter").value = selected?.to || "";
      route("loads");
      renderLoads();
    };
  });
}
function approveDispatchLaneAlert() {
  const draft = latestDispatchPlan?.laneAlertDraft;
  const notice = $("#dispatchNotice");
  if (!draft) return;
  const saved = readJSON(storageKeys.laneAlerts, []);
  const duplicate = saved.some(
    (alert) =>
      String(alert.origin || "").toLowerCase() ===
        String(draft.origin || "").toLowerCase() &&
      String(alert.destination || "").toLowerCase() ===
        String(draft.destination || "").toLowerCase() &&
      String(alert.equipment || "").toLowerCase() ===
        String(draft.equipment || "").toLowerCase(),
  );
  if (!duplicate) {
    saved.unshift({
      ...draft,
    ...
    writeJSON(storageKeys.laneAlerts, saved.slice(0, 12));
    scheduleAccountSync();
  }
  renderAlerts();
  if (notice) {
    ...
    notice.textContent = duplicate
      ? "That Lane Alert is already saved."
      : "Lane Alert saved. It is now watching for matching loads.";
  }
  toast(duplicate ? "Lane Alert already saved." : "Lane Alert saved.");
}
function initDispatchAssistant() {
  const form = $("#dispatchForm");
  const submit = $("#dispatchSubmit");
  const notice = $("#dispatchNotice");
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      const data = await apiRequest("/api/dispatch", {
        method: "POST",
        body: dispatchRequestFromForm(),
      });
      renderDispatchPlan(data);
      toast("Dispatch plan ready.");
    } catch (error) {
      const plan = $("#dispatchPlan");
      const welcome = $("#dispatchWelcome");
      if (welcome) welcome.hidden = true;
      ...
      if (notice) {
        notice.hidden = false;
        notice.textContent =
          error?.data?.error || error?.message || "AI Dispatch could not create a plan.";
      }
      toast(error?.data?.error || "AI Dispatch could not create a plan.");
    } finally {
      submit.disabled = false;
      submit.textContent = "Create dispatch plan";
    }
  });
  $("#approveDispatchLaneAlert")?.addEventListener(
    "click",
    approveDispatchLaneAlert,
  );
}
function renderTrustedPartners() {
  ...
  const availablePartners = adminPreviewMode ? trustedPartners : [];
  const filtered = availablePartners.filter((partner) => {
    const text = [
      partner.name,
      partner.category,
      partner.city,
  ...
  box.innerHTML = filtered.length
    ...
    : adminPreviewMode
      ...
      : '<div class="card empty-state"><h3>No verified partners available</h3><p class="muted">Only server-backed partner records will appear here.</p></div>';
...
async function renderAgentClassroom() {
  ...
  const agentAvatarSrc = (agent = {}) => {
    const explicit = String(
      agent.avatarUrl || agent.avatar || agent.imageUrl || "",
    ).trim();
    if (explicit) return explicit;
    const text = `${agent.id || ""} ${agent.name || ""} ${agent.role || ""} ${agent.emoji || ""}`.toLowerCase();
    if (/customer|shipper|pickup/.test(text))
      return "/assets/classroom-avatar-customer-support.svg";
    if (/service|provider|repair|parking|tow|insurance|hotel/.test(text))
      return "/assets/classroom-avatar-service-provider.svg";
    if (/broker|fleet|company|carrier/.test(text))
      return "/assets/classroom-avatar-broker-company.svg";
    if (/admin|administrator|moderator/.test(text))
      return "/assets/classroom-avatar-administrator.svg";
    return "/assets/classroom-avatar-driver-support.svg";
  };
  const agentAvatarMarkup = (agent = {}) => {
    const label = escapeHtml(agent.name || agent.id || "OpenClaw agent");
    const src = escapeHtml(agentAvatarSrc(agent));
    return `<span class="agent-avatar-shell"><img class="agent-avatar" src="${src}" alt="${label}" loading="lazy" decoding="async" /></span>`;
  };
    ...
    agentsBox.innerHTML = agents.length
      ? agents
          .map((agent) => {
            ...
            return `<article class="card agent-status-card"><div class="agent-card-hero">${agentAvatarMarkup(agent)}<div class="agent-card-copy"><h3>${escapeHtml(agent.name || agent.id || "Agent")}</h3><p>${escapeHtml(agent.role || "OpenClaw agent")}</p></div><span class="agent-pill ${view.className}">${view.label}</span></div><div class="agent-progress"><span style="width:${progress}%"></span></div><p>${escapeHtml(agent.task || "Ready for an assignment.")}</p><div class="agent-card-meta"><span>${Number(agent.sessions || 0)} sessions</span><span>${Number(agent.errors || 0)} errors</span><span>${escapeHtml(agent.model || "Not reported")}</span></div><small>Last active: <time datetime="${escapeHtml(agent.lastActiveAt || "")}">${escapeHtml(displayTime(agent.lastActiveAt))}</time></small></article>`;
          })
          .join("")
...
function bind() {
  window.addEventListener("hashchange", () => {
    ...
    if (
      ["dashboard", "bulletin-form"].includes(id) ||
      (target && !target.classList.contains("active"))
    )
      route(id);
  });
  $$("[data-route]").forEach((el) =>
    el.addEventListener("click", (e) => {
      ...
      // Some navigation tags lead to standalone public pages, not an in-app
      // screen. Preserve those URLs so the click always reaches its page.
      if (href.startsWith("/") && ["knowledge", "resources"].includes(routeId))
        return;
      if (
        routeId === "signin" &&
...
(async () => {
  ...
  const localPreviewAllowed = ["localhost", "127.0.0.1"].includes(
    location.hostname,
  );
  if (previewRole && (accountDashboardRoute === "admin" || localPreviewAllowed)) {
    activateAdminPreview(previewRole);
    renderAdminPreviewBanner(previewRole);
  }
  ...
  if (location.hash) {
    ...
    if (next === "admin" && !adminPreviewMode) {
      location.href = "/admin.html";
    } else if (adminPreviewMode) {
      route(
...
// --- Pickup post drafts: auto-save on this device until posted ---
(function initPostDraft() {
  const form = document.getElementById("postForm");
  if (!form) return;
  const KEY = "rm:postDraft:v1";
  const fields = () =>
    Array.from(form.querySelectorAll("[name]")).filter(
  ...
  const saveButton = document.getElementById("postSaveDraft");
  if (saveButton)
    saveButton.onclick = () => {
      saveDraft();
      if (typeof toast === "function") toast("Draft saved on this device.");
    };
  if (restoreDraft() && typeof toast === "function")
    toast("Draft restored. Finish the details and post when ready.");
})();

// --- Freight photos: resize locally before the load is submitted ---
(function initPostPhotos() {
  const input = document.getElementById("postPhotos");
  if (!input) return;
  renderPostPhotoPreview();
  input.addEventListener("change", async () => {
    ...
    if (files.length > 3) {
      ...
      postPhotoPayload = [];
      renderPostPhotoPreview();
      toast("Add no more than 3 freight photos.");
      return;
    }
    input.disabled = true;
    try {
      postPhotoPayload = await Promise.all(files.map(resizeFreightPhoto));
      renderPostPhotoPreview();
      if (postPhotoPayload.length) {
        toast(`${postPhotoPayload.length} freight photo${postPhotoPayload.length === 1 ? "" : "s"} ready.`);
      }
    } catch (error) {
      input.value = "";
      postPhotoPayload = [];
      renderPostPhotoPreview();
      toast(error?.message || "The photos could not be prepared.");
    } finally {
...
// --- Post composer: live preview + completion progress ---
(function initPostComposer() {
  ...
  const update = () => {
    ...
    const date = val("pickup_date");
    const time = val("pickup_time");
    setText("previewPickup", [date, time].filter(Boolean).join(" · "), "Not set");
    setText("previewEquipment", val("equipment_needed"), "Not set");
    const freight = [val("commodity"), val("weight"), val("dimensions")]
      .filter(Boolean)
      .join(" · ");
    ...
    setText("previewRate", rateLabel, "Not set");
    setText("previewPayment", val("payment_terms"), "Not set");
    ...
    setText(
      "postProgress",
      `Draft · ${percent}% complete`,
      "Draft · 0% complete",
    );
    const status = document.getElementById("postPreviewStatus");
    if (status) status.textContent = percent >= 100 ? "Ready to post" : "Draft";
  };
  form.addEventListener("input", update);
  form.addEventListener("change", update);
  // Initial paint (after draft restore, which runs before this block).
  update();
})();
